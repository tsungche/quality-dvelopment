...
class TeacherAdminService{
    ...
    public function getSuspending(){
        $suspending = $this -> getRecord(array(
            "t_record.status" => "待学院审批"
        ));
        for ($i = 0; $i < count($suspending); $i++) {
            $suspending[$i]["student_url"] = U('TeacherAdmin/student', array('type' => base64_encode($suspending[$i]['student_id'])));
        }
        return $suspending;
    }
    ...
    public function getSubjectReport($studentId = "",$where = array()){
        ...
        for ($i = 0; $i < count($subjectReport); $i++) {
            ...
            $subjectReport[$i]['adopt_val'] = base64_encode($subjectReport[$i]['id']);
            $subjectReport[$i]['detail_url'] = U('TeacherAdmin/detail', array('record' => base64_encode($subjectReport[$i]['id'])));
    ...
    public function getSimpleApproval(){
        ...
        for ($i = 0; $i < count($simple); $i++) {
            ...
            $simple[$i]["student_url"] = U('TeacherAdmin/student', array('type' => base64_encode($simple[$i]['student_id'])));
        }
        return $simple;
    }
     ...
     * @return mixed
     */
    public function getHandlingApproval(){
        $simple = $this -> getSubjectReport(null,array(
            "t_record.status" => "待商议"
        ));  //待商议报表
     ...
     * @return mixed
     */
    public function recordGo($recordId){
        $user = session(C('USER'));
                    ...
                    if($results){
                        ...
                        M('t_user_message')
                            ...
                            -> add(array(
                                ...
                                'type' => '学院消息',
                                'record_id' => $recordId,
                                'content' => "你的 <strong style='color:#199dc6;'>" .$record['title']."</strong> 已被 ".$user[C('NAME')]. " 审批完毕，结果为 <strong style='color:#c6c200;'>已通过</strong>",
     ...
     * @return mixed
     */
    public function handlingGo($data){
        $user = session(C('USER'));
        $record = $this -> getRecord(array(
            "t_record.id" => $data["recordId"],
            "t_record.status" => "待商议"
     ...
     * @return mixed
     */
    public function getStudentList($where,$page = 1){
        return M("t_student")
            ->where($where)
    ...
    public function report($page,$where = array()){
        $students = $this -> getStudentList($where,$page);
        $pageHtml = getPageHtml("TeacherAdmin/student",$this -> getStudentCount($where),$page,$where);
        for($i = 0;$i < count($students);$i++){
            ...
            $students[$i]["student_url"] = U('TeacherAdmin/student', array('type' => base64_encode($students[$i]['user_id'])));
        }
