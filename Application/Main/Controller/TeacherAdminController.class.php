...
class TeacherAdminController extends Controller {
    ...
    public function _initialize(){
        ...
        if($i_service -> hasLogin(C('TEACHERADMIN')) || $i_service -> hasLogin(C('SUPERADMIN'))){
    ...
    public function index(){
        $i_service = D('Index','Service');
        $t_service = D('TeacherAdmin','Service');
    ...
    public function student(){
        ...
        if(!empty($studentId)){  //单科目项报表
            ...
            $this->display("TeacherAdmin/report");
        }
    }
    ...
    public function detail(){
        ...
        $detail = D('TeacherAdmin','Service') -> getRecord(array(
        ...
        $type = D('TeacherAdmin','Service') -> getTypeById($detail[0]['type_id']);
    ...
    public function adopt(){
        $recordId = base64_decode(I('post.recordId'));
        $result = D('TeacherAdmin','Service') -> recordGo($recordId);
    ...
    public function failed(){
        $post = I("post.");
        $result = D('TeacherAdmin','Service') -> recordNotGo($post);
        $this -> ajaxReturn($result,'JSON');
    }

    //待商议审批通过
    public function handlingAdopt(){
        $post = I("post.");
        $result = D('TeacherAdmin','Service') -> handlingGo($post);
        $this -> ajaxReturn($result,'JSON');
    }
    ...
    //待商议审批
    public function handlingApproval(){
        $t_service = D('TeacherAdmin','Service');
        $record = $t_service -> getHandlingApproval();  //待处理报表
        $this->assign("record",$record);
        $this -> display();
    }
